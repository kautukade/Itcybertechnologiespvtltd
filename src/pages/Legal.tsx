...
const content: Record<string, { title: string; updated: string; intro: string; blocks: Block[] }> = {
  "/privacy-policy": {
    ...
    blocks: [
      ...
      { h: "4. Retention & your rights", p: [
        "Enquiry data is kept while the conversation and any engagement are active, then archived or deleted. You may request access, correction or deletion of your data at any time by emailing " + site.contact.email + ".",
      ]},
      { h: "5. Security", p: [
      ...
      { h: "6. Contact", p: [
        `Questions about this policy: ${site.contact.email}. This policy may be updated as our services evolve; the date above reflects the current version.`,
      ]},
    ],
  },
  "/terms-of-service": {
    ...
    blocks: [
      { h: "1. Our services", p: [
        "ITCYBER designs and delivers AI agents, automation systems, integrations and custom software. Scope, timelines and pricing for any engagement are defined in a written proposal; where a proposal and these terms conflict, the proposal prevails.",
      ]},
      ...
      { h: "6. Governing law", p: [
        "These terms are governed by the laws of India, with courts of the state of our registered office having jurisdiction. Questions: " + site.contact.email + ".",
      ]},
    ],
  },
  "/cookie-policy": {
    ...
    blocks: [
      ...
      { h: "3. Control", p: [
        "You can clear or block cookies in your browser settings at any time; the site remains fully usable without them. Questions: " + site.contact.email + ".",
      ]},
    ],
  },
};
...
export default function Legal() {
  ...
  const doc = content[pathname] ?? content["/privacy-policy"];
      ...
      <section className="relative bg-ink-950 text-ink-100 overflow-hidden noise">
        ...
        <div className="relative wrap pt-[clamp(3rem,6vw,5rem)] pb-[clamp(2.5rem,5vw,4rem)] max-w-3xl">
          <Reveal>
            ...
            <p className="font-mono text-[0.7rem] uppercase tracking-[0.16em] text-ink-400 mt-4">{doc.updated}</p>
            <p className="text-ink-200 mt-5 leading-relaxed">{doc.intro}</p>
          </Reveal>
        </div>
      </section>
      <Section tone="paper">
        <div className="wrap max-w-3xl space-y-8">
          {doc.blocks.map((b, i) => (
            <Reveal key={b.h} delay={i * 0.03}>
              <section>
                <h2 className="font-display font-bold text-ink-900 text-[1.25rem] tracking-tight">{b.h}</h2>
                {b.p.map((t) => (
